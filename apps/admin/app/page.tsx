  ...
  return (
    <AdminShell>
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-[#1A0706]">Clinic System Overview</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white border border-[#D9D9D9] rounded-2xl p-4 sm:p-5 space-y-4 shadow-sm">
          ...
          {recentActivity.length === 0 ? (
            ...
            <div className="space-y-2.5">
              {recentActivity.slice(0, 6).map((act, idx) => (
                <div key={idx} className="p-3 bg-slate-50 border border-[#D9D9D9] rounded-xl text-xs flex justify-between items-center gap-2">
                  <div className="min-w-0 flex-1">
                    <span className="font-bold text-[#DD0200] block sm:inline">{act.action}</span>
                    <p className="text-slate-750 font-bold truncate">By {act.actorEmail} ({act.actorRole})</p>
                  </div>
                  <span className="text-[10px] text-slate-500 font-bold shrink-0">{new Date(act.timestamp).toLocaleTimeString()}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="bg-white border border-[#D9D9D9] rounded-2xl p-4 sm:p-5 space-y-3 shadow-sm">
